...
import me.endr.toxic.event.events.UpdateWalkingPlayerEvent;
...
public class AutoCrystal
            ...
            if (packet2.getType() == 51 && AutoCrystal.mc.player.getDistanceSq(pos = new BlockPos(packet2.getX(), packet2.getY(), packet2.getZ())) + (double)this.predictOffset.getValue() <= MathUtill.square(this.breakRange.getValue()) && (this.instantTimer.getValue() == PredictTimer.NONE || this.instantTimer.getValue() == PredictTimer.BREAK && this.breakTimer.passedMs(this.breakDelay.getValue()) || this.instantTimer.getValue() == PredictTimer.PREDICT && this.predictTimer.passedMs(this.predictDelay.getValue()))) {
                ...
                if (this.predictFriendDmg.getValue() && (this.antiFriendPop.getValue() == AntiFriendPop.BREAK || this.antiFriendPop.getValue() == AntiFriendPop.ALL) && this.isRightThread()) {
                    for (EntityPlayer friend : AutoCrystal.mc.world.playerEntities) {
                        if (friend == null || AutoCrystal.mc.player.equals(friend) || friend.getDistanceSq(pos) > MathUtill.square(this.range.getValue() + this.placeRange.getValue()) || !Toxic.friendManager.isFriend(friend) || !((double) DamageUtill.calculateDamage(pos, friend) > (double) EntityUtill.getHealth(friend) + 0.5))
                            continue;
                        return;
                    }
                }
                if (placedPos.contains(pos.down())) {
                    float selfDamage;
                    if (this.isRightThread() && this.superSafe.getValue() ? DamageUtill.canTakeDamage(this.suicide.getValue()) && ((double) (selfDamage = DamageUtill.calculateDamage(pos, AutoCrystal.mc.player)) - 0.5 > (double) EntityUtill.getHealth(AutoCrystal.mc.player) || selfDamage > this.maxSelfBreak.getValue()) : this.superSafe.getValue()) {
                        return;
                    }
                    ...
                    if ((double) selfDamage + 0.5 < (double) EntityUtill.getHealth(AutoCrystal.mc.player) && selfDamage <= this.maxSelfBreak.getValue()) {
                        for (EntityPlayer player : AutoCrystal.mc.world.playerEntities) {
                            float damage;
                            if (!(player.getDistanceSq(pos) <= MathUtill.square(this.range.getValue())) || !EntityUtill.isValid(player, this.range.getValue() + this.breakRange.getValue()) || this.antiNaked.getValue() && DamageUtill.isNaked(player) || !((damage = DamageUtill.calculateDamage(pos, player)) > selfDamage || damage > this.minDamage.getValue() && !DamageUtill.canTakeDamage(this.suicide.getValue())) && !(damage > EntityUtill.getHealth(player)))
                                continue;
    ...
    private boolean isRightThread() {
        return mc.isCallingFromMinecraftThread() || !Toxic.eventManager.ticksOngoing() && !this.threadOngoing.get();
    }
    ...
    public void onKeyInput(InputEvent.KeyInputEvent event) {
        if (Keyboard.getEventKeyState() && !(AutoCrystal.mc.currentScreen instanceof ToxicGui) && this.switchBind.getValue().getKey() == Keyboard.getEventKey()) {
            if (this.switchBack.getValue() && this.offhandSwitch.getValue() && this.offHand) {
                Offhand module = Toxic.moduleManager.getModuleByClass(Offhand.class);
                if (module.isOff()) {
    ...
    private boolean check() {
        ...
        if (!((this.offHand || this.mainHand || this.switchMode.getValue() != Switch.BREAKSLOT || this.switching) && DamageUtill.canBreakWeakness(AutoCrystal.mc.player) && this.switchTimer.passedMs(this.switchCooldown.getValue()))) {
            this.renderPos = null;
            target = null;
            this.rotating = false;
            return false;
    ...
    private void mapCrystals() {
            ...
            if ((double) selfDamage + 0.5 < (double) EntityUtill.getHealth(AutoCrystal.mc.player) && selfDamage <= this.maxSelfBreak.getValue()) {
                ...
                for (EntityPlayer player : AutoCrystal.mc.world.playerEntities) {
                    ...
                    if (!(player.getDistanceSq(entity) <= MathUtill.square(this.range.getValue())))
                        continue;
                    if (EntityUtill.isValid(player, this.range.getValue() + this.breakRange.getValue())) {
                        if (this.antiNaked.getValue() && DamageUtill.isNaked(player) || !((damage = DamageUtill.calculateDamage(entity, player)) > selfDamage || damage > this.minDamage.getValue() && !DamageUtill.canTakeDamage(this.suicide.getValue())) && !(damage > EntityUtill.getHealth(player)))
                            continue;
                        if (damage > maxDamage) {
                            maxDamage = damage;
                            maxCrystal = entity;
                    ...
                    if (this.antiFriendPop.getValue() != AntiFriendPop.BREAK && this.antiFriendPop.getValue() != AntiFriendPop.ALL || !Toxic.friendManager.isFriend(player.getName()) || !((double) (damage = DamageUtill.calculateDamage(entity, player)) > (double) EntityUtill.getHealth(player) + 0.5))
                        continue;
        ...
        if (this.shouldSlowBreak(true) && maxDamage < this.minDamage.getValue() && (target == null || !(EntityUtill.getHealth(target) <= this.facePlace.getValue()) || !this.breakTimer.passedMs(this.facePlaceSpeed.getValue()) && this.slowFaceBreak.getValue() && Mouse.isButtonDown(0) && this.holdFacePlace.getValue() && this.holdFaceBreak.getValue())) {
            this.efficientTarget = null;
            return;
        }
        if (this.packets.getValue() == 1) {
    ...
    private void placeCrystal() {
        ...
        if (this.placeTimer.passedMs(this.placeDelay.getValue()) && this.place.getValue() && (this.offHand || this.mainHand || this.switchMode.getValue() == Switch.CALC || this.switchMode.getValue() == Switch.BREAKSLOT && this.switching)) {
            ...
            if (target != null && this.placePos != null) {
                if (!this.offHand && !this.mainHand && this.autoSwitch.getValue() != AutoSwitch.NONE && (this.currentDamage > (double)this.minDamage.getValue() || this.lethalSwitch.getValue() && EntityUtill.getHealth(target) <= this.facePlace.getValue()) && !this.switchItem()) {
                    return;
                }
                if (this.currentDamage < (double)this.minDamage.getValue() && this.limitFacePlace.getValue()) {
                    crystalLimit = 1;
                }
                if (this.currentDamage >= (double)this.minMinDmg.getValue() && (this.offHand || this.mainHand || this.autoSwitch.getValue() != AutoSwitch.NONE) && (this.crystalCount < crystalLimit || this.antiSurround.getValue() && this.lastPos != null && this.lastPos.equals(this.placePos)) && (this.currentDamage > (double)this.minDamage.getValue() || this.minDmgCount < crystalLimit) && this.currentDamage >= 1.0 && (DamageUtill.isArmorLow(target, this.minArmor.getValue()) || EntityUtill.getHealth(target) <= this.facePlace.getValue() || this.currentDamage > (double)this.minDamage.getValue() || this.shouldHoldFacePlace())) {
    ...
    private void calculateDamage(EntityPlayer targettedPlayer) {
        ...
        block0:
        for (BlockPos pos : BlockUtill.possiblePlacePositions(this.placeRange.getValue(), this.antiSurround.getValue(), this.oneDot15.getValue())) {
            ...
            if (!((double) selfDamage + 0.5 < (double) EntityUtill.getHealth(AutoCrystal.mc.player)) || !(selfDamage <= this.maxSelfPlace.getValue()))
                continue;
            if (targettedPlayer != null) {
                ...
                if (this.calcEvenIfNoDamage.getValue() && (this.antiFriendPop.getValue() == AntiFriendPop.ALL || this.antiFriendPop.getValue() == AntiFriendPop.PLACE)) {
                    ...
                    for (EntityPlayer friend : AutoCrystal.mc.world.playerEntities) {
                        float friendDamage;
                        if (friend == null || AutoCrystal.mc.player.equals(friend) || friend.getDistanceSq(pos) > MathUtill.square(this.range.getValue() + this.placeRange.getValue()) || !Toxic.friendManager.isFriend(friend) || !((double) (friendDamage = DamageUtill.calculateDamage(pos, friend)) > (double) EntityUtill.getHealth(friend) + 0.5))
                            continue;
                        friendPop = true;
                        break;
                    }
                ...
                if (this.foundDoublePop || !(playerDamage > maxDamage) && (!this.extraSelfCalc.getValue() || !(playerDamage >= maxDamage) || !(selfDamage < maxSelfDamage)) || !(playerDamage > selfDamage || playerDamage > this.minDamage.getValue() && !DamageUtill.canTakeDamage(this.suicide.getValue())) && !(playerDamage > EntityUtill.getHealth(targettedPlayer)))
                    continue;
                maxDamage = playerDamage;
                currentTarget = targettedPlayer;
                currentPos = pos;
            ...
            for (EntityPlayer player : AutoCrystal.mc.world.playerEntities) {
                float friendDamage;
                if (EntityUtill.isValid(player, this.placeRange.getValue() + this.range.getValue())) {
                    if (this.antiNaked.getValue() && DamageUtill.isNaked(player)) continue;
                    ...
                    if (this.foundDoublePop || !(playerDamage > maxDamage) && (!this.extraSelfCalc.getValue() || !(playerDamage >= maxDamage) || !(selfDamage < maxSelfDamage)) || !(playerDamage > selfDamage || playerDamage > this.minDamage.getValue() && !DamageUtill.canTakeDamage(this.suicide.getValue())) && !(playerDamage > EntityUtill.getHealth(player)))
                        continue;
                    maxDamage = playerDamage;
                    currentTarget = player;
                    currentPos = pos;
                ...
                if (this.antiFriendPop.getValue() != AntiFriendPop.ALL && this.antiFriendPop.getValue() != AntiFriendPop.PLACE || player == null || !(player.getDistanceSq(pos) <= MathUtill.square(this.range.getValue() + this.placeRange.getValue())) || !Toxic.friendManager.isFriend(player) || !((double) (friendDamage = DamageUtill.calculateDamage(pos, player)) > (double) EntityUtill.getHealth(player) + 0.5))
                    continue;
    ...
    private EntityPlayer getTarget(boolean unsafe) {
        ...
        EntityPlayer currentTarget = null;
        for (EntityPlayer player : AutoCrystal.mc.world.playerEntities) {
            if (EntityUtill.isntValid(player, this.placeRange.getValue() + this.range.getValue()) || this.antiNaked.getValue() && DamageUtill.isNaked(player) || unsafe && EntityUtill.isSafe(player))
                continue;
        ...
        switch (this.rotate.getValue()) {
            ...
            case ALL: {
                ...
                if (this.eventMode.getValue() == 2 && this.threadMode.getValue() == ThreadMode.NONE) {
                    Toxic.rotationManager.setPlayerRotations(angle[0], angle[1]);
                    break;
                }
                this.yaw = angle[0];
                this.pitch = angle[1];
    ...
    private boolean isDoublePoppable(EntityPlayer player, float damage) {
        float health;
        if (this.doublePop.getValue() && (double) (health = EntityUtill.getHealth(player)) <= this.popHealth.getValue() && (double) damage > (double) health + 0.5 && damage <= this.popDamage.getValue()) {
        ...
        public void run() {
            if (this.autoCrystal.threadMode.getValue() == ThreadMode.WHILE) {
                while (this.autoCrystal.isOn() && this.autoCrystal.threadMode.getValue() == ThreadMode.WHILE) {
                    while (Toxic.eventManager.ticksOngoing()) {
                    }
